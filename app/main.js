...
const server         = require('app/server/electronServer');
...
var appHost = null;               // The hostname to connect to the local node server we start.
var pendingPathToOpen = null;     // Path to open when app is started to open a document.
...
function onInstanceStart(argv, workingDir) {
  argv = cleanArgv(argv);
...
function openWindowForPath(path) {
...
function handleOpen(serverMethods, filepath) {
  ...
  switch (ext) {
    ...
    case '.xlsx':
    case '.xlsm':
      serverMethods.importDoc(filepath).then(docName => openWindowForPath(docName));
      break;
    default:
...
function notifyMigrateBackup(backupPath) {
  let msgBoxArgs = {
    type: "info",
    buttons: ["Ok"],
    message: "Backup Made",
  ...
  electron.dialog.showMessageBox(msgBoxArgs, () => null);
}
...
app.on('ready', function() {
  ...
  .then(serverMethods => {
    ...
    serverMethods.onDocOpen(filePath => {
    ...
    serverMethods.onBackupMade(bakPath => notifyMigrateBackup(bakPath));
